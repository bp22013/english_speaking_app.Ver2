...
export const updateMessage = new Hono().post('/updateMessage', async (c) => {
    try {
        ...
        if (targetMessage.length === 0) {
            return c.json({ flg: false, message: 'メッセージが見つからないか、編集権限がありません' }, 404);
        }

        const { content: oldContent, messageType: oldType, messagePriority: oldPriority, sentAt, scheduledAt: oldScheduledAt } = targetMessage[0];

        // 同じ内容・タイプ・優先度・送信時間のメッセージを全て更新
        const conditions = [
            eq(messages.senderId, senderId),
            eq(messages.content, oldContent),
